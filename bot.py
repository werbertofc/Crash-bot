    ...
    try:
        ...
        if len(command_parts) == 4:
            ...
            if process.returncode == 0:
                bot.reply_to(message, f"Ataque iniciado com sucesso no IP {ip} por {duration} segundos!")
            else:
                bot.reply_to(message, f"Erro ao iniciar o ataque no IP {ip}. Verifique os logs do sistema.")
        else:
